...
- **プライバシーポリシーはこのリポジトリで編集しない。** 正本は imakuru_app リポジトリの
  `プライバシーポリシー.md` で、GitHub Actions（`sync-privacy-policy.yml`）が
  keyanbaitian6-cell/imakuru-legal へ自動反映する。このリポジトリの `privacy.html` は
  `/imakuru-legal/` への転送ページであり、本文を書き戻さないこと（2026-08-16に一本化）。
- 位置情報・広告・外部サービスの扱いが変わった場合は、アプリ本体の実装、
  imakuru_app の `プライバシーポリシー.md`、ストアのデータセーフティ情報を同時に更新する。
